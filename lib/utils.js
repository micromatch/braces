...
utils.split = function(str) {
  ...
  while (segs.length) {
    ...
    while (key.slice(-1) === '\\' || (quoted = isQuote(key, next)) || (inside = insideExtglob(key, next))) {
      if (quoted) tok.quoted = true;
      if (inside) tok.inside = true;
      tok.escaped = true;
      var ch = segs.shift();
      if (!ch) break;
      if (inside) {
...
function insideExtglob(key, next) {
  return /\((?!.*\))/.test(key) && /\)/.test(next);
}
