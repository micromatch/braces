...
module.exports = function(brace) {
  ...
  brace.parser
    ...
    // .capture('brace.multiple', function() {
    //   var pos = this.position();
    //   var m = this.match(/^\{(,+)\}/);
    //   if (!m) return;
    //   console.log(m)
    ...
    //   var node = pos({
    //     type: 'brace',
    //     nodes: [open, multiplier, close]
    //   });

    //   utils.define(open, 'parent', node);
    //   utils.define(close, 'parent', node);
    //   utils.define(multiplier, 'parent', node);
    //   return node;
    // })
    ...
    .capture('text', function() {
      ...
      var prev = this.prev();
      var tok = pos({type: 'text', val: m[0]});
      if (tok.val.charAt(0) === '"' && tok.val.slice(-1) === '"') {
